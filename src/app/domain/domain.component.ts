...
export class DomainComponent implements OnInit {
  ...
  translate: any | {} = {
    ...
    title: { word: '_Domain', disabled: false, visible: true },
    popupTitle: { word: '_Domain', disabled: false, visible: true },
  ...
  onClick_button_edit(event: any) {
    ...
    this.gDomain = event;
    this.gDomain['task_id'] = this.config.keys.filter(
      (x: any) => x.list == 'Daily'
    )[0].keyList;
  ...
  onClick_button_confirm() {
    if (this.gForm.instance.validate().isValid) {
      this.config.isLoadingPanel = true;
      if (this.gDomainTemp.id === null) {
        this.sDomain.Post(this.gDomainTemp).subscribe({
          next: (data: any) => {
            this.config.isEdit = data.isEdit;
            this.config.isView = data.isView;

            Object.assign(this.gDomain, {
              id: data.id,
              name: data.name,
            ...
            this.dsDomian = [this.gDomain, ...this.dsDomian];
          },
          complete: () => {
            this.config.isLoadingPanel = false;
        ...
        this.sDomain.Put(this.gDomainTemp).subscribe({
          next: (data: any) => {
            Object.assign(this.gDomain, {
              name: data.name,
              isConfirmDelete: data.isConfirmDelete,
  ...
  onClick_button_confirmDelete(event: any) {
    ...
    this.sDomain
      ...
      .subscribe({
        next: () => {
          this.dsDomian = this.dsDomian.filter(
            (domain) => domain.id !== event.id
          );
        },
        complete: () => {
          this.config.isLoadingPanel = false;
